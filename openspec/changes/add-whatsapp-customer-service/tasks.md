...
- [ ] 3.3 放入少量已審核政策種子（加單截止、自取、惡劣天氣），並以搜尋測試確認這些問法能命中對應條目
...
- [ ] 5.1 實作規則優先的意圖分類（訂單號、改期／取消／投訴／議價／付款爭議、政策詞），分不清才用獨立限額的小模型抽 JSON 槽位，並以分類測試確認危險關鍵字不依賴模型
...
- [ ] 6.1 補齊 webhook 冪等、查單三態、意見寫入、FAQ 搜尋、轉手與殺開關的單元／函式測試，並以 `npx vitest run` 跑本變更相關測試通過
