create function public.report_monthly_prepared_meat_prices(
  report_year integer,
...
as $$
  with ranked_prices as (
    ...
    where price.month_at is not null
      and price_mode in ('shop', 'factory')
  )
  select
  ...
  where product.archived_at is null
    ...
    and price.selected_price is not null
  order by product.sort_order nulls last, product.name, price.price_month;
$$;

revoke all on function public.report_monthly_prepared_meat_prices(integer, text)
from public, anon;
grant execute on function public.report_monthly_prepared_meat_prices(integer, text)
to authenticated, service_role;

comment on function public.report_monthly_prepared_meat_prices(integer, text) is
  'Returns the latest migrated monthly shop or factory price for each active prepared-meat product, including its package conversion. Runs as invoker so source-table RLS applies.';
