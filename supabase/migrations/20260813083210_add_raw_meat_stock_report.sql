create function public.report_monthly_raw_meat_stock(
  report_year integer
)
returns table (
  ...
  month_end_kg numeric,
  monthly_net_kg numeric
)
language sql
stable
set search_path = ''
as $$
  ...
  select
    ...
    coalesce(
      sum(
        coalesce(movement.inbound_quantity_kg, 0)
        - coalesce(movement.outbound_quantity_kg, 0)
      ) filter (
  ...
  left join public.raw_meat_stock_movements as movement
    on movement.raw_meat_item_id = item.id
...
revoke all on function public.report_monthly_raw_meat_stock(integer)
from public, anon;
grant execute on function public.report_monthly_raw_meat_stock(integer)
to authenticated, service_role;

comment on function public.report_monthly_raw_meat_stock(integer) is
  'Returns cumulative raw-meat KG balances and monthly net movement by Hong Kong month. Runs as invoker so source-table RLS applies.';
