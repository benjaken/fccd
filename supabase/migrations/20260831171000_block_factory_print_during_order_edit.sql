-- Keep the database print marker protected as a second line of defence. The
-- factory UI blocks before sending data to the printer; this prevents a stale
-- or custom client from recording a print against an actively edited order.
...
begin
  ...
  perform public.assert_factory_order_printable(v_order_id);
...
grant execute on function public.mark_factory_order_line_printed(uuid) to authenticated;
