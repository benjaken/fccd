-- Central-kitchen product sales report.
-- Product metadata is read from the catalog while quantities and amounts come
-- from valid order lines delivered during the selected Hong Kong date range.
...
as $$
  with product_catalog as (
    select
      p.id as product_id,
      p.sku,
      coalesce(nullif(btrim(p.chinese_name), ''), p.name) as product_name,
    ...
    group by p.id, p.sku, p.chinese_name, p.name, ch.name, pt.name
  )
  select
    catalog.product_id,
    catalog.sku,
  ...
  join public.order_lines line on line.product_id = catalog.product_id
...
comment on function public.report_kitchen_product_sales(date, date, uuid, text, uuid) is
  'Aggregates central-kitchen product sales by product, brand, category and product collection for a Hong Kong delivery date range.';
