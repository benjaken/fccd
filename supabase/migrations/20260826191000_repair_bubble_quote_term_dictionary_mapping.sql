-- Repair the first import after verifying that Bubble term creation order does
-- not match the order of the five seeded dictionary values.
with legacy_rows as (
  select
    'term'::text as kind,
    legacy_id,
...
delete from public.dict_items as items
using public.dict_types as types
where types.id = items.dict_type_id
  and types.code = 'quote_term_template'
  and items.metadata->>'source' = 'bubble'
  and items.metadata->>'legacy_id' in (
    '1718874871317x302550465051099140',
    '1735319152080x494370356477034500'
  )
  and items.value not in ('no_refund', 'delivery_slots');
