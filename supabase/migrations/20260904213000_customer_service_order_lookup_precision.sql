...
create function public.customer_service_lookup_order_items(
  p_phone text,
  p_order_id uuid
)
returns table (
...
begin
  ...
  select
    ...
    case
      when nullif(btrim(line.content_snapshot), '') is distinct from coalesce(
        nullif(btrim(line.product_name_snapshot), ''),
        nullif(btrim(product.name), '')
  ...
  where line.order_id = p_order_id
    ...
    and (
      ...
      or nullif(btrim(line.new_quantity_text), '') is not null
    )
...
comment on function public.customer_service_lookup_order_items(text, uuid) is
  'Returns grouped order dish details only when the service-role caller supplies the WhatsApp phone that owns the order.';
