...
begin
  ...
  where (coalesce(p_famous_brand_only, false) = false or grouped.is_famous_brand = true)
...
create index if not exists customer_tag_assignments_email_idx
  on public.customer_tag_assignments (lower(btrim(customer_email_snapshot)));

comment on function public.list_quote_customers(text, text, boolean, integer, integer, boolean, uuid[]) is
  'Groups visible catering orders by email for the Quotes & Customers list, optionally limited to famous brand customers and selected customer tags.';

revoke all on function public.list_quote_customers(text, text, boolean, integer, integer, boolean, uuid[])
  from public, anon;
grant execute on function public.list_quote_customers(text, text, boolean, integer, integer, boolean, uuid[])
  to authenticated;
