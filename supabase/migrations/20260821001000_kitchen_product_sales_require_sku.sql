-- The legacy product-sales report is SKU based. Products without a SKU are
-- not single-product report rows and must not inflate the historical count.
...
comment on function public.report_kitchen_product_sales(date, date, uuid, text, uuid) is
  'Aggregates SKU-based single-product sales for a Hong Kong delivery date range.';
