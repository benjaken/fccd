-- The famous-brand customer view is a customer list, so an empty company
-- snapshot must not make an email-only customer appear in that view.
-- Also include the customer-tag representation used by historical backfills;
-- the old boolean flag is kept for backwards compatibility.
...
begin
        ...
        or exists (
          ...
          where lower(btrim(famous_tag.name)) = lower('知名品牌客戶')
            and (
              ...
              or lower(btrim(tag_assignment.customer_email_snapshot)) = lower(btrim(orders.email_snapshot))
            )
        )
      ) as is_famous_brand,
      coalesce(
  ...
  grouped as (
    select
      ...
      bool_or(
        source_orders.is_famous_brand
        and nullif(btrim(source_orders.company_name_snapshot), '') is not null
      ) as has_named_famous_brand,
      bool_or(source_orders.has_selected_famous_brand_tag) as has_selected_famous_brand_tag
    from source_orders
  ...
  where (
      coalesce(p_famous_brand_only, false) = false
      or grouped.has_named_famous_brand = true
    )
    and (cardinality(v_tag_ids) = 0 or grouped.has_selected_famous_brand_tag = true)
  order by
    case
...
comment on function public.list_quote_customers(text, text, boolean, integer, integer, boolean, uuid[]) is
  'Groups visible catering orders by email, optionally showing famous-brand customers with a named company only.';
