-- Keep each web-entered restaurant purchase form as its own record, even when
-- the restaurant, supplier, and business date are the same as another form.
alter table public.restaurant_supplier_purchases
  add column if not exists purchase_record_id uuid;

create index if not exists restaurant_supplier_purchases_purchase_record_id_idx
  on public.restaurant_supplier_purchases (purchase_record_id)
  where purchase_record_id is not null;

drop function if exists public.get_restaurant_daily_purchase_records(date, date, date, uuid[], uuid[], integer, integer);

create function public.get_restaurant_daily_purchase_records(
...
as $$
  ...
  order by
    ...
    grouped.supplier_name,
    grouped.restaurant_name,
    grouped.purchase_record_id nulls first
...
begin
  select * into strict restaurant_row
  ...
  -- Keep the legacy edit parameters for deployed clients. Only legacy rows
  -- without a purchase_record_id are eligible for that replacement behavior;
  -- a new web submission is always appended as a new record group.
...
drop function if exists public.get_restaurant_daily_purchase_entries(date, date, date, uuid[], uuid[], integer, integer);

create function public.get_restaurant_daily_purchase_entries(
  p_single_date date default null,
  p_start_date date default null,
  p_end_date date default null,
  p_restaurant_ids uuid[] default null,
