-- Persist the selected dishes for package lines created from quote/order drafts.
alter table public.production_calculations
  add column if not exists order_line_id uuid
  references public.order_lines(id) on delete cascade;

alter table public.order_package_choice_snapshots
  add column if not exists order_line_id uuid
  references public.order_lines(id) on delete cascade;

create index if not exists production_calculations_order_line_id_idx
  on public.production_calculations(order_line_id);

create index if not exists order_package_choice_snapshots_order_line_id_idx
  on public.order_package_choice_snapshots(order_line_id);
...
begin
  ...
  if p_item_kind = 'product' then
    ...
    select p.id, p.legacy_id, p.sku, coalesce(p.chinese_name, p.name)
...
grant execute on function public.add_quote_line(uuid,text,uuid,numeric,numeric,text,jsonb)
  to authenticated;
