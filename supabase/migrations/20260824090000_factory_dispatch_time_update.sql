...
begin
  ...
  if v_ship_out_time is null or v_ship_out_time !~ '^([01][0-9]|2[0-3]):[0-5][0-9]$' then
...
revoke all on function public.update_factory_order_dispatch_time(uuid, text) from public;
grant execute on function public.update_factory_order_dispatch_time(uuid, text) to authenticated;
