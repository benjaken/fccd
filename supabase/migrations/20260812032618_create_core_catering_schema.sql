...
create table public.restaurant_departments (
  id uuid primary key default gen_random_uuid(),
  legacy_id text not null unique,
  name text not null,
  sort_order integer,
...
alter table public.user_profiles
  add column shop_restro_id uuid references public.restaurants (id);

create index user_profiles_shop_restro_id_idx
  on public.user_profiles (shop_restro_id);
...
alter table public.restaurant_departments enable row level security;
...
revoke all on table
  ...
  public.delivery_districts,
  public.shipping_methods,
  public.order_statuses,
  public.order_tags,
  public.suppliers,
  public.restaurants,
  public.restaurant_departments,
  public.customers,
  public.products,
  public.packages,
...
comment on table public.orders is
  'Normalized Bubble A_Order headers. Customer, price, address and terms remain transaction snapshots.';
comment on table public.order_lines is
  'Normalized Bubble S_Order details. UUID relationships are resolved through legacy ID crosswalks.';
comment on table public.payments is
  'Normalized Bubble S_Payment records. Refund and void behavior must use future reversal records.';
comment on table public.deliveries is
  'Normalized Bubble B_delivery schedule records.';
