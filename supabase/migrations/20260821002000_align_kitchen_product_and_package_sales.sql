-- Product sales include product lines inside packages. Package sales are
-- counted separately from the package header line (product_id is null).
...
comment on function public.report_kitchen_product_sales(date, date, uuid, text, uuid) is
  'Aggregates SKU-based product sales, including products sold through packages, for a Hong Kong delivery date range.';
...
comment on function public.report_kitchen_package_sales(date, date, uuid, text, uuid) is
  'Aggregates package header sales separately from package product lines for a Hong Kong delivery date range.';
