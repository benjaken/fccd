-- Repair production drift: 20260831150000 recreated the 1-arg enqueue RPC after
-- 20260831133000 switched callers to (date, smallint). Edge Function posts both
-- p_reminder_date and p_reminder_hour, so PostgREST returns 404 when only the
-- 1-arg overload remains.
...
alter table public.driver_assignment_internal_reminder_outbox
  drop constraint if exists driver_assignment_internal_reminder_outbox_reminder_hour_check;

alter table public.driver_assignment_internal_reminder_outbox
  add constraint driver_assignment_internal_reminder_outbox_reminder_hour_check
...
alter table public.driver_assignment_internal_reminder_outbox
  drop constraint if exists driver_assignment_internal_re_reminder_date_channel_recipie_key;

alter table public.driver_assignment_internal_reminder_outbox
  drop constraint if exists driver_assignment_internal_reminder_outbox_slot_recipient_key;
...
drop function if exists public.enqueue_driver_assignment_internal_reminders(date, smallint);
