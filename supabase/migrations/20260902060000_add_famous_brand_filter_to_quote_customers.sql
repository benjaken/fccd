create or replace function public.list_quote_customers(
  ...
  p_famous_brand_only boolean default false
)
returns table (
  email text,
  customer_name text,
...
begin
  ...
  where coalesce(p_famous_brand_only, false) = false
    or grouped.is_famous_brand = true
  order by
    case
...
comment on function public.list_quote_customers(text, text, boolean, integer, integer, boolean) is
  'Groups visible catering orders by email for the Quotes & Customers list, optionally limited to famous brand customers.';
