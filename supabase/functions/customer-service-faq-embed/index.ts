...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (ids !== undefined && (!Array.isArray(ids) || ids.length > 50 || ids.some((id) => typeof id !== "string" || !/^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/i.test(id)))) return json({ error: "invalid_faq_ids" }, 400);
    const faqIds = (ids ?? []) as string[];
    if (payload.force === true && !faqIds.length) return json({ error: "force_requires_faq_ids" }, 400);
    ...
    const result = await runFaqEmbeddingBackfill({ db: admin, config, faqIds, force: payload.force === true, limit: requested });
    // Partial outcomes remain visible in a successful invocation; clients must inspect the body.
    return json({ ok: result.failed === 0 && result.databaseErrors === 0, ...result });
  } catch (error) {
    const code = error instanceof Error && error.message === "embedding_storage_dimension_mismatch" ? error.message : "embedding_job_failed";
    console.error("customer-service-faq-embed", code);
