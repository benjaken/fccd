...
function reconciliationIssueLabel(issueType: ReconciliationIssueRow["issue_type"]) {
  switch (issueType) {
    ...
    case "missing_delivery_date": return "沒有安排配送日期";
    case "kitchen_not_visible": return "廚房版面沒有顯示";
    case "driver_unassigned": return "沒有安排司機";
    case "insufficient_stock": return "存貨不足或物料設定不完整";
  }
}

function reconciliationIssueSummary(issues: ReconciliationIssueRow[]) {
  return [...new Set(issues.map((issue) => {
    if (issue.issue_type !== "insufficient_stock") {
      return reconciliationIssueLabel(issue.issue_type);
    }
    const shortageItems = Array.isArray(issue.metadata?.shortageItems)
      ? issue.metadata.shortageItems
      : [];
    const names = shortageItems.flatMap((item) => {
      if (!item || typeof item !== "object") return [];
      const name = (item as Record<string, unknown>).name;
      return typeof name === "string" && name.trim() ? [name.trim()] : [];
    }).slice(0, 5);
    const unmapped = Number(issue.metadata?.unmappedLineCount || 0);
    const detail = [
      names.length ? `短缺：${names.join("、")}` : "",
      unmapped > 0 ? `${unmapped}項未設定物料` : "",
    ].filter(Boolean).join("；");
    return detail || reconciliationIssueLabel(issue.issue_type);
  }))]
    .join("、");
}

function readinessOrderWatiParameters(
  order: OrderRow,
...
function reconciliationIssueGroups(issues: ReconciliationIssueRow[]) {
  ...
  return {
    missing: uniqueOrders(sorted.filter((issue) =>
      issue.issue_type === "missing_fccd" || issue.issue_type === "unlinked_fccd"
    )),
    factoryUnsent: uniqueOrders(sorted.filter((issue) => issue.issue_type === "factory_unsent")),
    readiness: uniqueOrders(sorted.filter((issue) => ![
      "missing_fccd",
      "unlinked_fccd",
      "factory_unsent",
...
function reconciliationEmailOrder(issue: ReconciliationIssueRow) {
  ...
  return `<li style="margin:0 0 14px">
    ...
    問題：${escapeHtml(reconciliationIssueLabel(issue.issue_type))}<br>
...
function reconciliationEmailContent(input: {
  ...
  if (!urgent && input.issues.length === 0) {
    return {
      ...
      html: `<div style="font-family:Arial,sans-serif;line-height:1.6">
        ...
        <h2>🎉 今日訂單核對完成！</h2>
        <p>今日沒有未入單、日期、廚房顯示、司機或存貨問題需要跟進，一切正常，祝工作順利！</p>
      </div>`,
    };
  }
  ...
  const dailySections = `<h3>【未入單】</h3>
    ...
    <h3>【訂單準備問題】</h3>
    ${groups.readiness.length ? `<ul>${groups.readiness.map(reconciliationEmailOrder).join("")}</ul>` : "<p>沒有</p>"}`;
  return {
    subject,
    html: `<div style="font-family:Arial,sans-serif;line-height:1.6">
...
async function sendWati(
  ...
  const destinationPhone = toNotificationWatiPhones([phone], recipientPolicy)[0] || "";
  if (!destinationPhone) {
...
async function sendEmail(
  ...
  const destination = toNotificationEmailRecipients([to], recipientPolicy)[0] || "";
  if (!destination) throw new Error("notification_recipient_allowlist_missing");
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { error: readinessRefreshError } = await admin.rpc(
      "refresh_order_readiness_issues",
      { p_now: nowIso },
    );
    if (readinessRefreshError && readinessRefreshError.code !== "PGRST202") {
      throw new Error(`order_readiness_refresh_failed:${readinessRefreshError.message}`);
    }
    ...
    for (const job of jobs) {
      ...
      if (!watiDone && (!automaticWatiEnabled || !notificationChannelEnabled(controls, template.event_key, "wati"))) {
        await admin.from("wati_order_notification_outbox").update({
        ...
        try {
          const wati = await sendWati(phone, template, parameters, controls.recipientPolicy);
          await admin.from("wati_order_notification_outbox").update({
      ...
      if (!emailDone && (!automaticEmailEnabled || !notificationChannelEnabled(controls, template.event_key, "email"))) {
        await admin.from("wati_order_notification_outbox").update({
    ...
    for (const job of internalJobs) {
      ...
      if (job.channel === "whatsapp" && (!automaticWatiEnabled || !notificationChannelEnabled(controls, "factory_unsent_reminder", "wati"))) {
        await admin.from("order_internal_notification_outbox").update({
          status: "skipped",
      ...
      if (job.channel === "email" && (!automaticEmailEnabled || !notificationChannelEnabled(controls, "factory_unsent_reminder", "email"))) {
        await admin.from("order_internal_notification_outbox").update({
          status: "skipped",
    ...
    for (const job of driverReminderJobs) {
      if (job.channel === "whatsapp" && (!automaticWatiEnabled || !notificationChannelEnabled(controls, "driver_assignment_reminder", "wati"))) {
      ...
      if (job.channel === "email" && (!automaticEmailEnabled || !notificationChannelEnabled(controls, "driver_assignment_reminder", "email"))) {
        await admin.from("driver_assignment_internal_reminder_outbox").update({
          status: "skipped",
          last_error: "email_automatic_notifications_disabled",
          locked_at: null,
    ...
    for (const job of reconciliationAlertJobs) {
      ...
      if (job.channel === "whatsapp" && (!automaticWatiEnabled || !notificationChannelEnabled(controls, "order_reconciliation", "wati"))) {
        await admin.from("order_reconciliation_alert_outbox").update({
          status: "skipped",
          last_error: "wati_automatic_notifications_disabled",
          locked_at: null,
      ...
      if (job.channel === "email" && (!automaticEmailEnabled || !notificationChannelEnabled(controls, "order_reconciliation", "email"))) {
        await admin.from("order_reconciliation_alert_outbox").update({
          status: "skipped",
          last_error: "email_automatic_notifications_disabled",
          locked_at: null,
