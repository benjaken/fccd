...
| 文件版本 | 2.0 |
| 文件日期 | 2026-09-01 |
| 文件狀態 | 現行實作基線；production 流程與權限持續驗收 |
| 範圍 | Catering 報價的新建／編輯、多步驟貨品與付款、轉正式訂單、工場設定及人工發送確認 |
| 參考 | 使用者提供截圖 1–9；目前 `QuoteEditorPage`、quote/order libraries、batch-save／conversion migrations 與測試 |
...
本 PRD 原先只定義截圖所示的第一步；v2.0 已擴展為現行多步驟編輯器基線。PDF 文件版面仍由 `QUOTE_LIST_AND_PDF_PRD.md` 定義；任何離開、轉單、傳工場或對客發送前，必須先保存有效資料。

### 1.1 2026-09-01 現行實作

1. 編輯器同時支援 quote 與由 quote 轉成的正式 order mode；步驟包含基本資料、貨品／套餐與付款／完成流程。
2. 基本資料支援客戶／公司／聯絡快照、第二電話、Email、Tag、品牌、運送方式、地址、地區、送貨日期、送貨時間及客戶備註；送貨時間可選設定值或自訂文字。
3. 貨品支援單品、套餐 choice set、自訂產品、零數量行、行備註、標籤預覽、Void、額外資訊、活動項目、餐具包、運費、CashDollar、折扣金額及可編輯折扣顯示文字。
4. 保存優先使用原子 batch-save；表頭、明細、套餐選擇、補充資料、付款及工場設定不得各自成功一半。
5. 報價可明確轉為正式訂單；轉單後不自動通知客戶，使用者核對正式訂單後另按「發送確認」。
6. 正式訂單可設定／取消傳送工場、禁止自動傳工場、抑制重印及工場打印日期；缺少工場必填資料時以 modal 阻擋。
7. 訂單編輯 presence 使用授權 Realtime 與 15 分鐘 session expiry；active edit session 可阻擋工場列印，避免列印未完成版本。
...
5. 在第 2 步按「下一步」時，系統驗證所有必填欄位、品牌相容性及貨品數量／金額。驗證成功後先原子保存報價與其子項目，再進入付款／完成步驟；失敗時留在原頁並將錯誤標示於對應欄位。
...
| --- | --- | --- |
| Quote／Order 共用編輯器 | 轉單後另開一套 editor 會令欄位、驗證和 totals 漂移；完全共用又容易在 quote 階段顯示不應出現的工場／通知操作。 | 共用資料與主要步驟，以 `document_type`／order mode 控制正式訂單才有的付款、工場及發送操作；共用計算與保存函式。 |
| 報價轉正式訂單 | 早期轉單後嘗試自動發確認，其後改回手動。 | 轉單是資料 lifecycle 操作，不是對客發送批准；成功轉單後由使用者檢查正式訂單，再另按發送確認。 |
| 金額與活動項目 | 活動、運費、折扣、CashDollar 在 editor、detail 與 PDF 曾使用不同小計。 | 有效產品行與活動項目由共用 breakdown 計算；Void 排除；折扣金額與可編輯 label 分開；所有視圖及文件使用同一 authoritative totals。 |
| 空白補充資料 | 額外資訊／活動為空時曾整段隱藏，使用者無法在空狀態新增。 | 可編輯模式永遠顯示入口與明確 empty state；read-only 可顯示「沒有」或按文件版面保留空區，但不得把空值誤判為載入失敗。 |
| 套餐 choice 與 Shopify helper lines | 套餐展開及 Shopify option 行曾重覆成正式商品，或失去 choice identity。 | 保存 package、choice set、selected product 及 line identity；helper／selection rows 不當成獨立收費商品；轉單與重開 editor 後選擇可還原。 |
| 數量驗證 | 為兼容取消／暫存行加入零數量後，與正式訂單整數要求出現衝突。 | 報價／訂單 editor 可保存業務允許的零數量行，但生產、標籤與 totals 按有效行規則處理；需要整數的正式出產數量在後端驗證。 |
| 工場設定 | 「不傳工場」、「不通知改動」、已傳送及重印標記容易互相覆蓋。 | 各旗標保存獨立語意；送工場前驗證必填資料並確保 delivery；已傳送後的修改按 change audit／reprint rule 處理，不靠 UI checkbox 靜默改寫狀態。 |
| 多人編輯 | 一般 Realtime 更新不能防止兩人同時保存或工場列印半成品。 | 使用 authenticated presence 與具期限 edit session；DB 是事實來源；active session 期間阻擋工場列印，高風險保存仍需 server validation。 |
...
7. Production 各角色何時可設定「不傳送至工場」與 suppress reprint，以及已傳送後修改的通知責任人。
8. 付款／完成步驟在不同品牌是否還需要額外欄位或審批；目前流程不得因新增品牌而硬編碼分支。
