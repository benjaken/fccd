...
| 文件版本 | 1.4 |
| 文件日期 | 2026-09-03 |
| 文件狀態 | 外部 WhatsApp 已確認接 WATI；外部品項來源仍未定。其餘 Phase 1 規則可進入 OpenSpec／實作規劃 |
...
| 參考 | `docs/API_FUNCTIONAL_PRD.md` §5.8 TRF、§5.11 ROS；`docs/SITEMAP.md`；`docs/architecture/functional-catalog.md`；`docs/PERMISSION_SITEMAP.md`；`docs/WATI_ORDER_NOTIFICATIONS.md` |
...
| 訂貨雙通道 | **外部供應商**→WATI 發 WhatsApp；**自己倉**→在 FCCD 登記成表格，走審核／工場。 |
| 審核 | 寫字樓審核；可**修改**申請，或**傳送工場**。店長不可批。 |
| 傳工場 | 先處理／修改，再**手動**傳送工場；不會自動發送。 |
| 截單 | 沒有每日截單。下單必填送貨日期。 |
| 價格 | 舖頭訂貨不顯示成本或轉撥價。 |
| 出貨 | 自己倉申請一次出完。 |
| 目錄 | 自己倉用現有凍貨／`ingredients` 加「分店可訂」。外部可訂品項來源**未定**（現有 `restaurant_ingredients` 或供應商自維簡表）。 |
...
2. 選該分類可訂品項、數量、送貨日期、備註。不顯示價格。品項目錄來源見 §16（未定）。
...
外部：經 WATI 發送 WhatsApp。自己倉：保存並送出登記。

### 8.3 供應商列表（訂貨分類）

寫字樓維護：名稱、分類（外部／自己倉）、啟用、可訂品項、排序。自己倉必須能掛現有凍貨／`ingredients` 的「分店可訂」。外部可訂品項來源未定：現有 `restaurant_ingredients`，或供應商列表自維簡表；選定前不要做第三套主檔。
...
全店佇列：通道、供應商、店舖、送貨日期、狀態。自己倉可點進審核；外部可看 WATI 發送狀態、範本、收件號碼與重試。
...
| 外部可訂品項 | **未定** | 現有 `restaurant_ingredients`，或供應商自維簡表。 |
...
3. **外部可訂品項**先用現有 `restaurant_ingredients`，還是供應商列表自維簡表。選定前實作可先接介面，不要同時建兩套主檔。
4. 新 WATI 範本名稱、參數順序與誰按發送（舖頭直接送，或寫字樓確認後才送）。建議：舖頭送出即入 outbox，失敗可重試。
