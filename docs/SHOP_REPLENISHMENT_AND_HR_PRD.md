...
| 文件版本 | 1.1 |
| 文件日期 | 2026-09-03 |
| 文件狀態 | 業務規則已確認大部分 Phase 1 決策；MPF 是否納入 Phase 2 仍待確認 |
...
| 餐廳員工主檔 | `/restaurant/staff` 只有姓名、電話、店舖、部門、僱傭類型、啟用 | 沒有假期申請、更表實作、打卡；MPF 是否需要仍待確認 |
...
6. 之後在同一分店工作區擴充 **Shop Manager** 營運與獨立角色 **Shop HR** 人事：同事列表、同事假期、更表、網頁打卡；MPF 是否納入待確認。
...
- Shop HR（獨立角色）：同事列表、同事假期、網頁打卡／出勤；MPF 預設不做，除非業務確認要記強積金供款。
...
| Shop HR | 獨立系統角色；管同事資料、假期、網頁打卡（Phase 2）。MPF 僅在業務確認後才納入。 |
...
| Accounting | 只讀申請、出貨與收貨差異（可另開報表權限） | 只讀工時匯出；MPF 若確認才開放 |
...
| `shop.hr`、`shop.hr.staff`、`shop.hr.leave`、`shop.hr.attendance` | Phase 2；`shop.hr.mpf` 僅在確認要做 MPF 後才加 |
...
結論：乾貨目錄用現有 `ingredients` +「分店可訂」。出貨必須新建乾貨流水。工場若要警示「夠不夠貨」，第一期可用最近一次盤點量作參考，並允許在沒有可靠即時結餘時仍出貨（寫出貨記錄），但預設仍警示數量為零或未盤點。不把出貨回寫成一筆假盤點。
...
**MPF 是什麼：** 香港的「強制性公積金」。僱主通常要按工資替僱員每月供款，並保留僱員部分、僱主部分、工資基數等紀錄。它不是打卡，也不是更表，而是勞工／會計文件。

初稿因原需求提到「MPF紀錄」而列入。**尚未確認店舖人事是否真的要在 FCCD 記這個。** 若只是要同事、假期、更表、打卡，Phase 2 可不做 MPF。

若確認要做，範圍如下（手記紀錄，不做法定自動計算或電子申報）：
...
5. 私人附件（收貨異常照片；若確認做 MPF 則含憑證）走 private bucket，不可公開 URL。
...
3. **MPF（強積金供款紀錄）要不要做。** 不做的話 Phase 2 只留同事、假期、更表、網頁打卡。
4. 乾貨沒有即時結餘時，工場出貨是「僅警示仍可出」還是「沒有最近盤點就不准出」。建議前者。

## 16. 建議下一步

1. 回覆 §15（尤其 MPF 與乾貨出貨政策）。
