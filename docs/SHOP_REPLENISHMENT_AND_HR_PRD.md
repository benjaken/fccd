...
| 文件版本 | 1.0 |
| 文件日期 | 2026-09-03 |
| 文件狀態 | 草案；待業務確認審核人、截單時間、目錄來源及 Phase 2 HR 細則後進入實作 |
| 所屬模塊 | 餐廳／分店營運、凍貨、乾貨、工場、人力資源 |
| 主要使用者 | Shop Manager、Shop HR、FCCD 審核人員、工場、Accounting、Admin／Super Admin |
...
| 餐廳員工主檔 | `/restaurant/staff` 只有姓名、電話、店舖、部門、僱傭類型、啟用 | 沒有假期申請、MPF、更表、打卡 |
...
1. 餐廳在一個頁面同時訂**凍貨**與**乾貨**（可含半成品），指定數量、單位、需要日期及備註。
...
6. 之後在同一分店工作區擴充 **Shop Manager** 營運與 **Shop HR** 人事：同事列表、同事假期、MPF 紀錄、更表、打卡。
...
- Shop Manager：本店同事、更表、訂貨與日常營運入口。
- Shop HR：同事列表、同事假期、MPF 紀錄、打卡／出勤。
...
- 客戶自助、司機代收、QR 硬件考勤機對接（Phase 2 先做系統內打卡）。
...
| Shop HR | 分店人事角色或模組；管同事資料、假期、MPF、打卡（Phase 2）。 |
...
| 角色 | Phase 1 | Phase 2 |
| --- | --- | --- |
| Shop Manager | 建立／送出本店申請、查看本店狀態、確認收貨 | 本店同事、更表、假期申請覆核 |
| Shop HR（新角色） | 預設不開放訂貨審核或中央庫存 | 同事列表、假期、MPF、打卡；預設只看獲授權店舖 |
| Factory | 只處理**已發送工場**的執貨／出貨 | 不處理人事 |
| Admin／Super Admin | 審核全部店、管理可訂目錄、查看存貨與出貨 | 可覆核跨店人事設定 |
| Accounting | 只讀申請、出貨與收貨差異（可另開報表權限） | 只讀 MPF／工時匯出 |
| Company User | 依 page rows；不可審核或改庫存 | 依授權 |

約束：

1. Shop Manager **不可**批核自己送出的申請（除非另授獨立 `shop.replenishment.review` 且業務書面批准）。
2. 工場**不可**把待審申請執貨或扣倉。
...
對應 Sitemap「餐廳訂貨」。Shop Manager 預設只見本店。

**列表欄位：** 申請編號、店舖、建立者、需要日期、凍貨項數、乾貨項數、狀態、審核者、工場發送時間、出貨／收貨狀態、操作。

**篩選：** 狀態、需要日期範圍、關鍵字（編號／品項）。

**新建／編輯（草稿）：**

| 欄位 | 規則 |
| --- | --- |
| 店舖 | 必填；Shop Manager 鎖定本店 |
| 需要日期 | 必填；香港日曆；不可早於今天（截單規則見 §10） |
| 備註 | 選填 |
| 明細 | 至少一行；可混凍貨與乾貨 |
| 品項 | 只可選「分店可訂」且啟用的目錄 |
| 數量 | 必須 > 0；遵守品項小數位與單位 |
...
- 批核成功後可立即「發送到工場」，或留在 `approved` 待批次發送。
- 拒絕整張必須填原因，分店可見。
...
工場可見：店舖、需要日期、已核准行、倉區（凍房／乾貨倉）、建議批次、列印執貨單。

工場不可見：未發送工場的申請、被拒行、成本價（除非另授權）。

操作：開始執貨 → 輸入實發量 → 確認出貨。實發量不可超過核准量，除非有「超發」權限（本期預設不開放超發）。
...
凍貨優先寫入現有 `raw_meat_stock_movements`／`prepared_meat_stock_movements`（或同等 RPC），並帶補貨申請參照。乾貨若尚無等價 movement 表，本期建立乾貨倉流水，不把數量寫進餐廳盤點事件冒充入貨。
...
| --- | --- |
| 出貨編號 | 系統產生 |
| 申請編號 | 一張申請可分多次出貨（分批），但第一期可限制一次出完 |
...
| 編號 | 需求 | 優先 |
| --- | --- | --- |
| REP-01 | 分店可建立補貨申請，指定店舖、需要日期、備註，以及凍貨／乾貨明細（品項、數量、單位、行備註）。 | P0 |
...
| REP-07 | 發送到工場是明確動作（可與批核同一交易，但狀態與審計分開）。 | P0 |
...
| REP-14 | 庫存不足時工場看到警示；是否允許負庫存由據點＋品項政策控制，預設不允許。 | P1 |
| REP-15 | FCCD 可維護可訂目錄（凍貨／乾貨、單位、最低訂量、排序、暫缺）。 | P1 |
| REP-16 | 店舖訂貨數量報告改為（或並列）讀取本補貨文件的已核准／已出貨量，避免只反映舊 `meat_orders`。 | P1 |
...
不把補貨寫進到會 `orders`。`meat_orders` 保留給現有凍肉客戶／出貨單；新流程用獨立文件，需要時再同步或淘汰舊路徑。

| 實體 | 用途 |
| --- | --- |
| `shop_replenishment_orders` | 申請頭：店舖、申請人、需要日期、狀態、審核者、發送工場時間、備註 |
...
| `shop_shipments`／`shop_shipment_lines` | 出貨記錄 |
| `shop_shipment_exceptions` | 收貨異常 |
| 現有凍貨 movement | 凍貨出倉／回沖 |
| 新乾貨 movement（若無） | 乾貨倉出倉／在途／回沖 |
...
下列為本草案假設；標「待確認」者實作前必須由業務拍板。

| 規則 | 假設 | 待確認 |
| --- | --- | --- |
| 適用店舖 | 所有 `restaurants` 啟用店，不限將軍澳 | 哪些店第一批上線 |
| 截單 | 需要日期至少為下一個香港工作日 | 每日截單鐘點、假日是否順延 |
| 審核人 | Admin／Super Admin／持 `shop.replenishment.review` 的營運 | 是否指定專人、是否要雙審 |
| 批核後發送 | 允許「批核即發送」或「批核後手動發送」 | 預設哪一種 |
| 價格 | 分店下單頁**不顯示**成本／內部價 | 是否之後要顯示轉撥價 |
| 最低訂量 | 目錄可設；第一期可空 | 各品項 MOQ |
| 分批出貨 | 第一期一張申請一次出完 | 是否允許分批 |
| 代用品 | 第一期不允許工場擅自換貨 | 缺貨時是否建議替代品 |
| 與 `meat_orders` | 新單走新表；舊報告仍讀舊表直到 REP-16 | 歷史凍肉店舖單是否遷移 |
...
| 編號 | 需求 |
| --- | --- |
| HR-08 | 按同事、月份保存 MPF 供款紀錄：僱員部分、僱主部分、工資基數、狀態、備註、憑證附件。 |
| HR-09 | 不做法定自動計算引擎；可手輸入或匯入，保留誰改了什麼。 |
| HR-10 | 預設僅 Shop HR、Accounting、Admin 可見；Shop Manager 是否可見待確認。 |
...
| HR-16 | 記錄時間、店舖、來源（網頁／手機）、操作者；後期才接實體機。 |
...
| `report_shop_order_quantities` | 現讀 `meat_orders`；REP-16 後應對準新申請 |
...
1. Shop Manager 可建立混合凍貨＋乾貨申請並送審；其他店資料不可見。
2. 待審單在工場不可見；批核並發送後工場才可見已核准行。
...
1. 第一批上線店舖名單與 Shop Manager 帳號對應。
2. 審核角色：只限寫字樓營運，還是店長可批其他店。
3. 每日截單時間、最早／最晚需要日期。
4. 可訂目錄來源：現有凍貨主檔＋廚房食材，或另建分店目錄。
5. 批核後預設是否立即發工場。
6. 分店下單是否顯示任何價格或轉撥價。
7. 乾貨倉現有盤點能否直接當結餘來源，或要先補乾貨 movement。
8. 舊 `meat_orders` 店舖單是否停止新增，以及報告何時切換。
9. Phase 2：Shop HR 是獨立角色，還是 Shop Manager 的子權限包。
10. 打卡是否必須定位／只限店內網絡；MPF 欄位是否要對齊現有 Excel。

## 16. 建議下一步

1. 業務確認 §10 與 §15。
2. 以本 PRD 開 OpenSpec change（例如 `add-shop-replenishment`），產出 design／delta specs／tasks 後再實作 Phase 1。
3. Phase 2 另開 change，避免與訂貨狀態機綁在同一實作批次。
