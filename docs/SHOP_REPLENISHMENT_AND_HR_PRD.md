...
| 文件版本 | 1.7 |
| 文件日期 | 2026-09-03 |
| 文件狀態 | 清單供應商已對 FCCD `suppliers`。能對上的掛現有 ID；浚峰、益力多未建檔。 |
...
| 鳳香園麵飽有限公司 | 鳳香園 | 簡稱 | — | |
| 淘大 | 淘大 | 全名相同 | — | 主檔 `is_active=false`；訂貨仍可用，或先啟用 |
| 浚峰企業有限公司 | — | **未對上** | — | 主檔無此公司，實作前要新建或確認別稱 |
| 益力多公司 | — | **未對上** | — | 主檔無此公司，實作前要新建 |
...
| 供應商對照 | **已確認** | 能對上現有 `suppliers` 的用現有 ID。FC 兩倉都掛 `FC-總部`。浚峰、益力多未建檔。 |
...
3. 浚峰企業、益力多要在 `suppliers` **新建**，還是另有別稱。
4. 淘大主檔已停用，訂貨要不要重新啟用。
5. 浚峰兩行同名手套要保留兩行還是合併。
6. 其餘外部供應商缺電話的要補進電話簿（鴻發、日鮮、新豐、三潤、長明、強記已有主檔電話）。
7. 新 WATI 範本名稱、參數順序與誰按發送。建議：舖頭送出即入 outbox。
