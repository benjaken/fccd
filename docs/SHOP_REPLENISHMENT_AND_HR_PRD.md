...
| 文件版本 | 2.2 |
| 文件日期 | 2026-09-03 |
| 文件狀態 | Phase 1 規則已齊。Phase 2 應徵名單欄位已依舊 App 截圖鎖定；疫苗接種記錄不做。其餘 HR 內頁仍待截圖。 |
...
舊 App 選單**沒有**獨立的假期、更表、MPF。這三項仍暫列需求（見下），入口可能在員工詳情或其他頁；等內頁截圖後再定。

### 12.0 舊 App 選單對照

| 舊 App | Phase 2 | 現況 | 要不要內頁截圖 |
| --- | --- | --- | --- |
| 主頁 | 餐廳版面首頁 | 有殼即可 | 若有儀表數字／捷徑才要 |
| 應徵名單 | 新 | **已有截圖**（§12.7） | 夠用 |
| 員工名單 | 延伸 `restaurant_staff` | 有粗需求 | **要** 列表＋一筆員工詳情 |
| 本月打卡記錄 | 網頁打卡＋本月紀錄 | 有粗需求 | **要** 紀錄頁＋怎樣打卡 |
| 舖頭報數 | Phase 1 每日銷售 | 已有 | **不用** |
| 守則及告示 | 新 | 無 | **要** 列表＋一篇內容 |
| 員工生日日曆 | 新（多半跟員工出生日期） | 無 | 有獨立日曆頁才要 |
| 升職及加薪記錄 | 新 | 無 | **要** 列表＋一筆 |
| 疫苗接種記錄 | **不做** | — | **不用** |
...
| HR-04 | 停用不得刪歷史假期、MPF、打卡、更表、加薪與報稅紀錄。 |

欄位以員工詳情截圖為準（生日、緊急聯絡、職位等）。

### 12.2 同事假期

現有 holidays 設定只是類別。舊 App 選單無獨立入口，可能在員工詳情。
...
要做。手記／匯入，不做法定計算或申報。舊 App 選單無獨立入口，可能在員工詳情或報稅資料旁。
...
紙本申請上還有：通勤分鐘、哪裡得知、住家電話、家庭狀況、推薦人、兼職可用時間、銀行戶口／銀行名稱。預設**只當掃描附件**，不另做結構化欄，除非之後員工名單截圖證明要拆欄。
...
4. 假期、更表、MPF：舊 App 選單沒有獨立項。仍做的話，入口在哪（員工詳情／另頁）？
5. 報稅資料只是手存欄位／附件，還是要出 IR56 檔？（預設：只手存，不出檔。）
6. 常用聯絡人是否包含寫字樓／工場內部電話，確認與供應商電話簿完全分開。
7. 應徵「試工」之後怎樣轉正？預設：再按一次結果輸入改「聘用」。
8. 紙本申請的銀行／家庭狀況等：預設只存掃描，不拆欄。

## 17. 建議下一步

1. 以本 PRD 開 OpenSpec change `add-shop-replenishment`（餐廳版面＋雙通道訂貨＋貨倉入出）。
2. Phase 2 HR 另開 change。應徵名單已定（§12.7）。其餘仍先收齊 §12.0 內頁截圖。
