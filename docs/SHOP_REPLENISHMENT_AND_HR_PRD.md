...
| 文件版本 | 3.2 |
| 文件日期 | 2026-09-03 |
| 文件狀態 | Phase 1 餐廳訂貨、工場貨倉、分店確認收貨已落地。Shop HR 暫緩。 |
...
| 2.7 | 2026-09-03 | **不做守則及告示。** 舊 App 選單有此項，FCCD 不做、不遷、不補截圖。 |
...
| Shop HR | 獨立角色；Phase 2。選單對齊舊 FC Shop App。**不做疫苗接種記錄、守則及告示、不獲聘用名單。** 打卡＝主頁一按即記。MPF 要做（手記）。常用聯絡人含辦公室／供應商／維修；供應商與訂貨電話簿同一套。 |
...
6. Phase 2 在餐廳版面補 Shop Manager／Shop HR。選單對齊舊 FC Shop App（見 §12）；疫苗接種記錄、守則及告示、不獲聘用名單不做。
...
Shop Manager／獨立 Shop HR。選單對齊舊 FC Shop App（應徵、員工、打卡、生日、升職加薪、常用聯絡人、報稅）。**不做疫苗接種記錄、守則及告示、不獲聘用名單。** 舖頭報數＝Phase 1 每日銷售。假期／更表／MPF 仍列需求，入口待截圖確認。
...
| WH-06 | 每日香港時間 00:00 以未來 14 日 FC 內部補貨單及所有渠道正式客戶訂單的食材／包裝 BOM 比較現有庫存並寄出摘要；即使沒有不足亦寄出正常摘要。 | P0 |
| WH-07 | 每個貨倉品項、食材及包裝用品可設定最低庫存；現存量首次跌至或低於門檻時觸發電郵，回升後再次跌穿才重發。 | P0 |
...
└─ Shop HR        對齊舊 App 選單
   ...
   ├─ 員工生日日曆
   ├─ 升職及加薪記錄
   ├─ 常用聯絡人        ← 辦公室同事／供應商／維修；供應商＝訂貨電話簿
   ├─ 報稅資料
   └─ 設定
```

**明確不做：** 疫苗接種記錄、守則及告示、不獲聘用名單。
...
主頁「打卡」按一下即寫一筆，香港時間、操作者、其店與組別。不開表單、不走 WATI、不定位。當日尚無上班＝記上班；已有上班未下班＝記下班。再按由主管補打處理（HR-17）。失敗要提示，成功後「本日打卡記錄」該組不再顯示「今日未打卡」。

| 編號 | 需求 |
| --- | --- |
| HR-15 | 網頁打上班／下班。入口在**主頁「打卡」**，按一下即記，沒有下一頁。 |
| HR-16 | 不接考勤機、不強制定位。 |
| HR-17 | 主管可補打卡（要原因與審計）。 |
...
| 編號 | 頁 | 暫定 |
| --- | --- | --- |
| HR-19 | 應徵名單 | 見 §12.7。未入職申請人；聘用並勾「送出到員工名單」後轉員工。不聘用／無出現留在應徵。 |
| HR-20 | 不獲聘用名單 | **不做。** |
| HR-21 | 守則及告示 | **不做。** |
| HR-22 | 員工生日日曆 | 見 §12.9。月曆顯示在職員工生日，資料來自員工檔月日。 |
| HR-23 | 升職及加薪記錄 | 手記職位／薪金變更；不是出糧。 |
| HR-24 | 常用聯絡人 | 見 §12.11。辦公室同事、供應商、維修。不走 WATI。 |
...
疫苗接種記錄（含舊入職表「疫苗記錄(針)」）、守則及告示、不獲聘用名單、自動出糧、IR56 電子申報、MPF 電子申報、未授權跨店調班、用假期類別頁冒充請假。HR 不阻擋 Phase 1 訂貨。
...
3. **打卡**  
   主頁大鈕「打卡」按一下即記（§12.5），沒有下一頁。  
   「本日打卡記錄」：店 × 組別卡（報數／廚房／店長／水吧）。無紀錄顯示「今日未打卡」。

Shop Manager 只看本店；Shop HR／Admin 可看授權店。活動與打卡都不是到會訂單。

### 12.11 常用聯絡人（已對截圖）

店務電話簿，三類可篩、可搜。點卡調系統電話或裝置 WhatsApp（不走 WATI）。沒號碼不能打。

| 類 | 卡片 | 資料從哪來 |
| --- | --- | --- |
| 辦公室同事 | 職稱／部門（小字）＋稱呼 | 這頁專用（寫字樓／營運等人），不是餐廳員工名單 |
| 供應商 | 公司名 | **同一套** Phase 1 供應商＋電話簿，不另建第二份 |
| 維修 | 公司名＋服務說明 | 這頁專用（隔油池、維修商等） |

舊畫面未見新增鈕。第一期：能看、篩、搜、打電話／WhatsApp。寫字樓／Admin 維護辦公室與維修；供應商跟訂貨電話簿一起改。新增表單可後補。
...
6. 常用聯絡人已定：辦公室同事＋供應商（共用訂貨電話簿）＋維修。
7. 應徵「試工」之後怎樣轉正？預設：員工名單改狀態為在職，或應徵再選聘用。
8. 紙本申請的銀行／家庭狀況等：員工詳情未拆欄，只存掃描。
9. MPF：主頁提醒交表＋滿 60 日才需供款。按月供款是否在報稅頁？
10. 打卡已定：主頁一按即記。樓面是否算在「報數」組仍可後補。
11. 「新增活動」表單請補一張。

## 17. 建議下一步

1. Phase 1 已開工：餐廳版面＋舖頭訂貨＋寫字樓審核。下一步工場貨倉入／出。
2. Phase 2 HR 暫緩，規格仍見 §12。
