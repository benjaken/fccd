...
| 文件版本 | 1.9 |
| 文件日期 | 2026-09-03 |
| 文件狀態 | Phase 1 主要規則已齊。WATI 發給外部供應商電話簿；審核用現有 Admin；缺電話之後再補。 |
...
| 外部 WhatsApp | **接 WATI**。收件人是該筆單選中的**外部供應商電話簿號碼**，不是客人、工場或到會內部提醒。沒有電話不能發，之後再補。 |
...
3. 「發送 WhatsApp」：經現有 **WATI** 發到該供應商電話簿上的號碼（可選其中一個聯絡人）。訊息內容為店名、送貨日期、品項與數量摘要。
4. **只發給外部供應商。** 不發給客人、司機、工場、寫字樓內部名單，也不走到會訂單提醒範本。
5. 沒有可用電話：需求仍可保存；發送按鈕停用，等之後補電話簿再發。
6. 系統把該需求記進訂貨紀錄（`wati_queued`／`wati_sent`／`wati_failed`／`wati_blocked_no_phone`）。**不傳工場、不扣自己倉。**
7. 實際付款／入帳仍用現有每日採購。範本未核准或未啟用時 fail closed。重試不得變成第二張需求。
...
聯絡人、電話、所屬供應商、備註。一個供應商可多個號碼；發 WATI 時可選，**收件人就是這個號碼**。沒有可用電話則不能發送，之後再補。號碼格式須符合現有 WATI 發送規則。主檔已有電話的可預填（鴻發、日鮮、新豐、三潤、長明、強記）。
...
| 貨倉入＋出 | 已確認 | 工場貨倉兩邊都做。 |
| 警示可出 | 已確認 | 不足仍可出。 |
| WhatsApp 發送管道 | **已確認** | 接 WATI。收件人＝該筆外部供應商電話簿號碼。沒電話之後補，其間不能發。 |
...
1. 浚峰兩行同名手套預設留兩行。
2. 缺電話的供應商之後提供；有號碼前該供應商不能發 WATI。
3. 新 WATI 範本名稱與參數順序：在 WATI 後台開核准範本後再填入系統。預設舖頭送出即入 outbox。
