...
| 文件版本 | 1.5 |
| 文件日期 | 2026-09-03 |
| 文件狀態 | 訂貨分類改為 FC 內部／外部供應商；可訂目錄以《餐廳供應商產品清單》為準。清單 Excel 尚未匯入工作區，欄位待對。 |
...
1. 選「FC 內部」分類（自己倉／FCCD 倉供應商）。
2. 只可選該清單上標記為 FC 內部的品項；同一張表可混凍貨／乾貨。能對上現有凍貨／`ingredients` 的用來扣倉，對不上的仍可下單但工場出貨要警示。
...
可訂目錄的業務來源是《餐廳供應商產品清單》（本機路徑參考：`餐廳供應商產品清單.xlsx`）。系統應可匯入／對照該清單，不要另外發明一套與清單無關的廚房食材目錄當舖頭選項。

- **外部供應商**行：掛在該供應商下，WATI 叫貨用。
- **FC 內部**行：掛在 FC 內部供應商下；能對到 `raw_meat_items`／`prepared_meat_items`／`ingredients` 的寫上對應 ID 以便貨倉扣帳。
- 現有 `restaurant_ingredients` 可作匯入對照，**不是**舖頭訂貨的唯一事實來源。

雲端工作區目前**讀不到**該 Excel。匯入欄位（供應商名、分類、品項、單位、規格等）待檔案上傳後補進本節。
...
| 可訂目錄 | **已確認方向** | 《餐廳供應商產品清單》。Excel 欄位待上傳後對。 |
...
3. 請把《餐廳供應商產品清單》Excel **上傳到對話或 repo**，以便對供應商名、FC 內部／外部標記、品項、單位、規格欄。雲端目前讀不到本機 `Downloads` 路徑。
