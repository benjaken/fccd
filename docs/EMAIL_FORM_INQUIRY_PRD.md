...
3. 列表欄位用**對應報價欄**取值，沒有則 `—`。待報價使用獨立查詢單號（`ENQYYYYMMDD-xxxxxx`），**不是**報價單號 `FCCQ…`：
   ...
   | 8 | 對客確認 | 無電郵／尚未寄出／已寄出／失敗 |
   | 9 | Asana | 有連結則顯示可點「Asana Link」；尚未建立或失敗顯示對應狀態 |
   | 10 | 操作 | 進入詳情。第一期無 PDF／複製／轉成訂單 |
