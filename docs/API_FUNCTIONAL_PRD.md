...
| UX-13 | 所有介面文字、導航、表單、驗證、通知、空狀態及 **輸入框 placeholder** 使用 i18n key，不得在元件內散落硬編碼文案；`zh-HK` placeholder 須為繁中或「例如：…」，不得使用英文 Bubble 原文。 |
...
- 輸入框 placeholder 必須使用 i18n key。預設 `zh-HK` 用繁體中文或數字範例（「例如：0.5」），禁止英文句子或 Bubble 原文（如 `Product Name`、`Choose some options...`）。English locale 才使用英文 placeholder。
