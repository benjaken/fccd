...
本文件不能單獨證明 Bubble 頁面 UI、workflow 內部動作、公式、排程、角色權限與第三方設定。這些項目仍需後續盤點，才能決定保留、重設、淘汰或歷史唯讀範圍及最終驗收案例。
...
1. 以舊系統 API 作能力盤點，保留約 30–40% 仍符合現況的邏輯，其餘按實際營運重新設計。
...
6. 提升資料讀取速度、資訊密度與操作效率，並為自動化及 AI 輔助報價預留安全擴充點。

### 3.2 產品原則與重設邊界

- 新業務流程優先於舊 Bubble 畫面、資料表及 workflow 形式。
- 每項舊能力在實作前分類為：**保留、重設、淘汰、歷史唯讀**。
- 101 種 Data Type 與 46 個 workflow 是盤點清單，不代表全部照原樣實作。
- 舊系統行為只在經業務確認仍適用時作平行驗收。
- 排更 API 雖存在於 Swagger，但會議確認排更不在現行系統；因此列為新增／待定範圍，不屬舊系統等價要求。
- 自助落單屬後期能力，不得擴大首階段核心交付範圍。

### 3.3 成功標準

- 101 種舊資料類型均有「遷移、合併、歷史唯讀、封存或不遷移」的明確 mapping。
- 46 個 workflow 均標記為保留、重設、淘汰或歷史唯讀，並經產品負責人確認。
- 角色與資料範圍權限通過匿名、本人、跨客戶、跨店舖及管理員測試。
- 實際遷移資料通過筆數、關聯、檔案、付款總額及未付金額對帳；未遷移歷史資料可經受控查詢層分析。
- 訂單、付款、庫存、配送與第三方 webhook 的重複請求不產生重複副作用。
- 核心頁面符合大字、少跳頁、少滾動及資料區最大化原則。
...
會議確認排更不在現行餐廳系統。下列需求來自 Swagger 線索，只作後續新增範圍候選，不列入首階段舊系統重設。
...
首階段若不實作完整總帳，介面名稱使用「財務對帳」，避免誤稱完整會計系統。
...
| 需求編號 | 產品需求 |
|---|---|
| WF-01 | 每個被分類為保留或重設的 workflow 必須定義 request schema、required、enum、範圍與關聯存在性；淘汰或歷史唯讀項目不建立新寫入入口。 |
| WF-02 | 被保留或重設 workflow 的成功回應包含 operation ID、異動 ID／筆數及業務結果，不只回傳空 object。 |
...
每類資料須記錄更新頻率、source of truth、保留期限及 Bubble 停用後的存取方案。
...
首階段採 responsive PWA 或原生 APP 須在技術驗證後決定；無論載體，使用相同 Auth、RLS、功能權限及稽核模型。
...
10. 若後續里程碑確認納入排班，同一員工重疊時段排班會被拒絕或要求具權限覆核；此項不作首階段驗收。
...
1. 每種來源 type 均有營運遷移、必要歷史遷移、歷史唯讀、封存或不遷移決策，以及 mapping、轉換與例外規則。
...
- 新系統不原樣複製，舊邏輯只作約 30–40% 參考。
- 自助落單後置；首階段先建立登入、設定、Layout 與核心模組 Spec。

### 14.2 需求追蹤模板

| 編號 | 認知／需求 | 狀態 | 待討論／決策 | 負責人 | 目標里程碑／日期 | 完成日期 | 未完成原因／下一步 |
|---|---|---|---|---|---|---|---|
| 示例 | 確認 Shopify Store 清單及 API owner | 待確認 | Leo 或 Yoko 提供認證資料 | 待指定 | 待指定 | — | 取得 Store access |

每項功能進入開發前建立追蹤列；完成後填寫實際完成日期與負責人。未完成項目由負責人設定目標里程碑／日期及下一步，不在本 PRD 中虛構時間估算。
...
6. 手機介面首階段採 responsive PWA，還是需要 App Store／Play Store 原生應用。

## 16. 實作階段

1. **登入、設定與 Layout**：建立 Auth、角色框架、一級頁面、頂部二級選單、共用資料表與表單模式。
2. **系統盤點與 Spec 凍結**：取得 Bubble Editor、Privacy Rules、option sets、workflow、Shopify 設定、狀態矩陣、公式及各角色測試帳號。
3. **基礎資料與權限**：建立品牌、Store、據點、倉區、客戶、商品、套餐、店舖、migration 及 RLS 測試。
4. **到會核心**：重設報價、訂單、最低額、Shopify、多品牌、生產扣料及附加服務。
5. **凍肉模組**：批次、投入、產量、收成率、香料及真實成本。
6. **司機與配送**：接拒、重派、配送狀態、照片、餐具及即時後台。
7. **餐廳報數**：每日申報、付款核對、工時、月成本及 P&L。
8. **內部調撥與營運分析**：將軍澳補貨、舊資料受控同步、聚合報表及 Recharts 儀表板。
9. **其他整合與後期能力**：WATI、PayPal、Asana／Zapier、自助落單及 AI 報價。
10. **驗收與切換**：按保留／重設能力執行案例，完成資料對帳、增量同步、監控及回復方案。

## 17. PRD 結論

Swagger 足以確認舊系統暴露 101 種資料能力及 46 個 workflow 入口，涵蓋 Catering 訂單、商品套餐、付款、配送、肉類加工、庫存、店舖營運、採購、排班與通知；這是盤點範圍，不是新系統全部必做清單。

會議確認新系統不是 100% 原樣複製：舊 API 是能力盤點基線，實際產品以中央廚房、分店調撥、多品牌 Shopify、凍肉真實成本、司機配送及餐廳報數的現行營運為主。後續須逐項確認 Bubble 內部邏輯與業務規則，再決定保留、重設、淘汰或歷史唯讀。
