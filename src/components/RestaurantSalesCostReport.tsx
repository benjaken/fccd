    ...
    () =>
      !loading && validRange && restaurantId
        ? {
            ...
            currentAggregates: months.map((month) => ({
              monthStart: month.monthStart,
              restaurantName: month.restaurantName,
              sales: month.sales.total,
              openingStock: month.opening.total,
              purchases: month.purchases.total,
              closingStock: month.closing.total,
              costOfSales: month.costOfSales.total,
              grossProfit: month.grossProfit.total,
            })),
            detailRows: months.flatMap((month) =>
              month.suppliers.map((supplier) => ({
                monthStart: month.monthStart,
                supplierName: supplier.supplierName,
                restaurantPurchases: supplier.restaurant,
                waterBarPurchases: supplier.waterBar,
                miscPurchases: supplier.misc,
                totalPurchases: supplier.total,
              })),
            ),
            completeness: {
              ...
              notes: [
                "銷售成本公式為期初存貨加採購減期末存貨。",
                "當前月份可能未完成盤點，期末存貨不足會扭曲銷售成本。",
              ],
            },
          }
        : null,
    [endMonth, loading, months, restaurantId, startMonth, validRange],
  );
