  ...
  return (
    <section className="ingredients-page packing-stocktakes-page">
      ...
      <div className="stocktake-records-layout">
        ...
        {!loading && total > 0 ? <TablePagination summary={t(copyKey("pagination"), { from: visibleFrom, to: visibleTo, total })} page={page} totalPages={totalPages} loading={loading} onPrevious={() => setPage((current) => Math.max(1, current - 1))} onNext={() => setPage((current) => Math.min(totalPages, current + 1))} onPageChange={setPage} previousLabel={t(copyKey("previous"))} nextLabel={t(copyKey("next"))} pageLabel={t(copyKey("pageOf"))} jumpLabel={t(copyKey("jumpToPage"))} /> : null}
