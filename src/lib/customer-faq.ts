...
export type CustomerServiceLearningSuggestion = {
  ...
  title: string;
  reason: string;
  proposedContent: { question?: string; answer?: string; category?: string; keywords?: string };
...
export async function fetchCustomerServiceDailyReports(limit = 14): Promise<CustomerServiceDailyReport[]> {
  const { data, error } = await supabase.rpc("customer_service_daily_reports_list", { p_limit: limit });
  if (error) throw error;
  return ((data ?? []) as Array<Record<string, unknown>>).map((row) => ({
    id: String(row.id), reportDate: String(row.report_date), environment: String(row.environment),
    ...
    failureThemes: Array.isArray(row.failure_themes) ? row.failure_themes as CustomerServiceDailyReport["failureThemes"] : [],
    aiSummary: String(row.ai_summary || ""), model: typeof row.model === "string" ? row.model : null,
    status: String(row.status || "partial"), error: typeof row.error === "string" ? row.error : null,
    generatedAt: String(row.generated_at),
  }));
}

export async function fetchCustomerServiceLearningSuggestions(status = "draft", limit = 50) {
  const { data, error } = await supabase.rpc("customer_service_learning_suggestions_list", { p_status: status, p_limit: limit });
  if (error) throw error;
  return ((data ?? []) as Array<Record<string, unknown>>).map((row): CustomerServiceLearningSuggestion => ({
    id: String(row.id), reportId: String(row.report_id), reportDate: String(row.report_date),
    ...
    proposedContent: row.proposed_content && typeof row.proposed_content === "object" ? row.proposed_content as CustomerServiceLearningSuggestion["proposedContent"] : {},
    evidenceCount: Number(row.evidence_count || 0), status: String(row.status || "draft"),
    targetFaqId: typeof row.target_faq_id === "string" ? row.target_faq_id : null, createdAt: String(row.created_at),
  }));
}

export async function generateCustomerServiceDailyReport(reportDate: string) {
  const { data, error } = await supabase.functions.invoke("customer-service-daily-report", { body: { report_date: reportDate } });
  if (error) throw error;
  return data;
}

export async function reviewCustomerServiceLearningSuggestion(id: string, status: "approved" | "rejected") {
  const { data, error } = await supabase.rpc("customer_service_learning_suggestion_review", { p_id: id, p_status: status });
  if (error) throw error;
...
export async function fetchCustomerServiceReviewTurns(reviewState = "unreviewed", limit = 50) {
  const { data, error } = await supabase.rpc("customer_service_turns_review_list", { p_review_state: reviewState, p_limit: limit });
  if (error) throw error;
  return ((data ?? []) as Array<Record<string, unknown>>).map((row): CustomerServiceReviewTurn => ({
    id: String(row.id), createdAt: String(row.created_at), question: String(row.question || ""),
    answer: typeof row.answer === "string" ? row.answer : null, intent: typeof row.intent === "string" ? row.intent : null,
    route: typeof row.route === "string" ? row.route : null, processingStatus: String(row.processing_status || ""),
    aiOutcome: typeof row.ai_outcome === "string" ? row.ai_outcome : null, aiReason: typeof row.ai_reason === "string" ? row.ai_reason : null,
...
export async function submitCustomerServiceTurnFeedback(input: { turnId: string; verdict: "correct" | "incorrect" | "needs_review"; failureCategory?: string; correctedAnswer?: string; note?: string; createFaqDraft?: boolean }) {
  const { data, error } = await supabase.rpc("customer_service_turn_feedback_submit", {
    p_turn_id: input.turnId, p_verdict: input.verdict, p_failure_category: input.failureCategory || null,
    p_corrected_answer: input.correctedAnswer || null, p_note: input.note || null,
...
export async function fetchCustomerServiceConfigVersions(environment = "develop") {
  const { data, error } = await supabase.rpc("customer_service_config_versions_list", { p_environment: environment });
  if (error) throw error;
  return ((data ?? []) as Array<Record<string, unknown>>).map((row): CustomerServiceConfigVersion => ({
    id: String(row.id), environment: String(row.environment), version: Number(row.version), label: String(row.label),
    model: String(row.model), systemPrompt: String(row.system_prompt || ""), temperature: Number(row.temperature),
    retrievalLimit: Number(row.retrieval_limit), status: row.status as CustomerServiceConfigVersion["status"],
    activatedAt: typeof row.activated_at === "string" ? row.activated_at : null, createdAt: String(row.created_at),
  }));
}

export async function createCustomerServiceConfig(input: { environment?: string; label: string; model: string; systemPrompt: string; temperature: number; retrievalLimit: number }) {
  const { data, error } = await supabase.rpc("customer_service_config_create", {
    p_environment: input.environment || "develop", p_label: input.label, p_model: input.model,
    p_system_prompt: input.systemPrompt, p_temperature: input.temperature, p_retrieval_limit: input.retrievalLimit,
  });
  if (error) throw error;
  return String(data);
}
...
export async function evaluateCustomerServiceConfig(id: string, sampleSize = 20) {
  const { data, error } = await supabase.functions.invoke("customer-service-model-evaluate", { body: { config_id: id, sample_size: sampleSize } });
  if (error) throw error;
  return data;
}
...
export async function fetchCustomerServiceEvaluationRuns(limit = 20) {
  ...
  return ((data ?? []) as Array<Record<string, unknown>>).map((row): CustomerServiceEvaluationRun => ({
    id: String(row.id), candidateConfigId: String(row.candidate_config_id), candidateLabel: String(row.candidate_label),
    candidateModel: String(row.candidate_model), status: String(row.status), sampleSize: Number(row.sample_size || 0),
    metrics: (row.metrics || {}) as Record<string, number>, error: typeof row.error === "string" ? row.error : null,
