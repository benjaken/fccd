...
import { DICT_TYPE, fetchDictItems } from "@/lib/dictionaries";
...
export async function fetchRestaurantStocktakeMasters(): Promise<RestaurantStocktakeMasters> {
  const [restaurantsResult, departmentItems] = await Promise.all([
    supabase
      .from("restaurants")
      .select("id,name")
      .eq("is_active", true)
      .is("archived_at", null)
      .order("name"),
    fetchDictItems(DICT_TYPE.restaurantStocktakeDepartment),
  ]);
  ...
  return {
    ...
    departments: departmentItems.map((department) => ({ id: department.id, name: department.label })),
  };
}
